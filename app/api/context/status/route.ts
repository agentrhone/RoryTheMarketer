...
export async function GET(req: NextRequest) {
  ...
  for (const section of ALL_SECTIONS) {
    switch (section.type) {
      ...
      case "swipe-files": {
        const lib = readContextLibrary(brand);
        status[section.id] = lib.items.length > 0;
        break;
      }
      case "meta-comments": {
      ...
      case "ad-account": {
        // Always "complete" — it's read-only config
        const b = getBrand(brand);
        status[section.id] = !!(b?.domain || b?.metaAdAccountId);
        break;
      }
      case "placeholder": {
        status[section.id] = false;
